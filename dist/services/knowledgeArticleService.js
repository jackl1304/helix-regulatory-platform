...
export class KnowledgeArticleService {
    ...
    async collectKnowledgeArticles() {
        try {
            ...
            const collectionResults = await Promise.allSettled(this.sources.map(source => this.processKnowledgeSource(source)));
            let totalArticles = 0;
    ...
    async checkIfArticleExists(article) {
        try {
            ...
            return allUpdates.some(existing => existing.url === article.url ||
...
//# sourceMappingURL=knowledgeArticleService.js.map
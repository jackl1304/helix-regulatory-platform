...
export class CrossReferenceService {
            ...
            for (const [groupKey, updates] of Object.entries(deviceGroups)) {
                ...
                for (let i = 0; i < updates.length; i++) {
                    for (let j = i + 1; j < updates.length; j++) {
                        const similarity = await this.calculateSimilarity(updates[i].title + ' ' + updates[i].content, updates[j].title + ' ' + updates[j].content);
    ...
    async mapStandardsToRegulations() {
        try {
            ...
            for (const standard of knownStandards) {
                ...
                const applicableRegulations = [...new Set(applicableUpdates.map(update => `${update.authority} - ${update.type}`))];
    ...
    async linkClinicalStudiesToApprovals() {
        try {
            ...
            const clinicalStudies = allUpdates.filter(update => update.type?.toLowerCase().includes('clinical') ||
                update.content.toLowerCase().includes('clinical study') ||
                update.content.toLowerCase().includes('clinical trial'));
            const approvals = allUpdates.filter(update => update.type?.includes('510(k)') ||
            ...
            for (const study of clinicalStudies) {
                ...
                if (!studyDeviceName && !studyManufacturer)
                    continue;
                ...
                for (const approval of approvals) {
                    ...
                    if (studyDeviceName && approvalDeviceName) {
                        confidence = Math.max(confidence, await this.calculateSimilarity(studyDeviceName, approvalDeviceName));
                    }
                    if (studyManufacturer && approvalManufacturer) {
                        confidence = Math.max(confidence, await this.calculateSimilarity(studyManufacturer, approvalManufacturer));
                    }
...
//# sourceMappingURL=crossReferenceService.js.map
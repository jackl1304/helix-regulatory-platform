...
export class NewsletterExtractionService {
    constructor() {
        this.logger = new Logger('NewsletterExtractionService');
        this.newsletterSources = [
            {
                id: 'bvmed_newsletter',
    ...
    async extractFromAllNewsletterSources() {
        ...
        const activeNewsletterSources = this.newsletterSources.filter(source => source.priority === 'high' && source.rssUrl);
    ...
    async saveNewsletterToKnowledgeBase(article, source) {
        try {
            ...
            const isDuplicate = existingArticles.some(existing => existing.title === article.title &&
...
//# sourceMappingURL=newsletterExtractionService.js.map
...
export class PredictiveAnalyticsService {
    ...
    async getHistoricalData(request) {
        try {
            ...
            if (request.deviceCategory) {
                filteredData = filteredData.filter(item => this.matchesDeviceCategory(item, request.deviceCategory));
            }
            if (request.manufacturer) {
                filteredData = filteredData.filter(item => this.matchesManufacturer(item, request.manufacturer));
            }
            if (request.jurisdiction) {
                filteredData = filteredData.filter(item => item.region?.toLowerCase().includes(request.jurisdiction.toLowerCase()) ||
            ...
            return filteredData.sort((a, b) => new Date(b.published_at || b.filed_date || 0).getTime() -
    ...
    predictSafetyAlerts(data, timeHorizon) {
        const predictions = [];
        const safetyAlerts = data.filter(item => this.isSafetyRelated(item.title + ' ' + item.content));
        ...
        for (const deviceType of deviceTypes.slice(0, 3)) {
            const deviceAlerts = safetyAlerts.filter(alert => this.matchesDeviceCategory(alert, deviceType));
    ...
    predictApprovals(data, timeHorizon) {
        const predictions = [];
        const approvals = data.filter(item => this.isApprovalRelated(item.title + ' ' + item.content));
        ...
        for (const jurisdiction of jurisdictions.slice(0, 3)) {
            const jurisdictionApprovals = approvals.filter(approval => approval.authority === jurisdiction);
    ...
    predictRegulatoryChanges(data, timeHorizon) {
        const predictions = [];
        const regulatoryUpdates = data.filter(item => this.isRegulatoryChange(item.title + ' ' + item.content));
        ...
        for (const tech of emergingTechs) {
            const techUpdates = data.filter(item => this.matchesTechnology(item, tech));
    ...
    predictMarketTrends(data, timeHorizon) {
        const predictions = [];
        const marketEvents = data.filter(item => this.hasMarketImpact(item.title + ' ' + item.content));
        predictions.push({
    ...
    calculateFrequency(data, timeHorizon) {
        ...
        const sortedData = data.sort((a, b) => new Date(a.published_at || a.filed_date || 0).getTime() -
    ...
    assessDeviceSafetyImpact(deviceType) {
        ...
        if (highRiskDevices.some(risk => deviceType.includes(risk)))
            return 'critical';
        if (mediumRiskDevices.some(risk => deviceType.includes(risk)))
            return 'high';
        return 'medium';
    }
    ...
    async identifyRiskFactors(request, data) {
        const riskFactors = [];
        const highPriorityItems = data.filter(item => item.priority === 'high' || item.priority === 'critical');
    ...
    generateRecommendations(predictions, riskFactors) {
        ...
        const highImpactPredictions = predictions.filter(p => p.impactLevel === 'high' || p.impactLevel === 'critical');
    ...
    assessJurisdictionRisk(data) {
        ...
        const highPriorityCount = recentData.filter(item => item.priority === 'high' || item.priority === 'critical').length;

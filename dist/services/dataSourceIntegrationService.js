...
export class DataSourceIntegrationService {
    ...
    async runComprehensiveDataCollection() {
        ...
        try {
            ...
            result.success = result.errors.length < result.servicesRun.length / 2;
    ...
    async runScheduledCollections() {
        ...
        const now = new Date();
        const collectionsToRun = this.scheduledCollections.filter(sc => sc.enabled && sc.nextRun <= now);
    ...
    calculateNextRun(from, frequency) {
    ...
    async updateCollectionSchedules() {
        try {
    ...
    async getIntegrationMetrics() {
        try {
            ...
            const weeklyUpdates = regulatoryUpdates.filter(u => u.publishedAt && new Date(u.publishedAt) >= weekAgo).length;
            const monthlyUpdates = regulatoryUpdates.filter(u => u.publishedAt && new Date(u.publishedAt) >= monthAgo).length;
...
//# sourceMappingURL=dataSourceIntegrationService.js.map
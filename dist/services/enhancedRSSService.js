...
export class EnhancedRSSService {
    ...
    async monitorAllFeeds() {
        try {
            ...
            const results = await Promise.allSettled(this.feeds.map(feed => this.processFeed(feed)));
    ...
    extractTags(item, feed) {
        ...
        const title = item.title.toLowerCase();
        const description = item.description.toLowerCase();
        if (title.includes('approval') || description.includes('approval'))
            tags.push('approval');
        if (title.includes('recall') || description.includes('recall'))
            tags.push('recall');
        if (title.includes('guidance') || description.includes('guidance'))
            tags.push('guidance');
        if (title.includes('safety') || description.includes('safety'))
            tags.push('safety');
        if (title.includes('device') || description.includes('device'))
            tags.push('medical_device');
        if (title.includes('software') || description.includes('software'))
            tags.push('software');
    ...
    async checkIfItemExists(regulatoryUpdate) {
        try {
            ...
            return allUpdates.some(existing => existing.url === regulatoryUpdate.url ||
...
//# sourceMappingURL=enhancedRSSService.js.map
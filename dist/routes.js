    ...
    app.get('/api/knowledge-articles', async (req, res) => {
        try {
            ...
            if (sourceFilter === 'newsletter') {
                articles = allArticles.filter(article => (article.source && article.source.toLowerCase().includes('newsletter')) ||
                    (article.authority && ['FDA News & Updates', 'EMA Newsletter', 'MedTech Dive'].includes(article.authority)) ||
                    (article.tags && article.tags.some((tag) => tag.toLowerCase().includes('newsletter'))));

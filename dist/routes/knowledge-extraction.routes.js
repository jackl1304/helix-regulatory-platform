...
router.get('/status', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const autoExtractedArticles = articles.filter(article => article.tags.includes('auto-extracted'));
        const needsReviewArticles = articles.filter(article => article.tags.includes('needs-review'));
        const status = {
            ...
            extractionPotential: {
                unprocessedRegulatory: regulatoryUpdates.filter(update => !articles.some(article => article.title.includes(update.title.substring(0, 50)))).length,
                unprocessedLegal: legalCases.filter(legalCase => !articles.some(article => article.title.includes(legalCase.title.substring(0, 50)))).length
            }
        };
...
router.post('/extract-all', asyncHandler(async (req, res) => {
...
router.post('/extract-sources', asyncHandler(async (req, res) => {
...
router.get('/preview', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const extractableRegulatory = regulatoryUpdates
            ...
            .filter(update => !existingArticles.some(article => article.title.includes(update.title.substring(0, 50))))
            .map(update => ({
            id: update.id,
            title: update.title,
        ...
        const extractableLegal = recentLegalCases
            ...
            .filter(legalCase => !existingArticles.some(article => article.title.includes(legalCase.title.substring(0, 50))))
            .map(legalCase => ({
            id: legalCase.id,
...
router.delete('/auto-extracted', asyncHandler(async (req, res) => {
...
//# sourceMappingURL=knowledge-extraction.routes.js.map